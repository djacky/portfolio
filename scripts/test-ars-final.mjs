// Simulate the exact ARSAgent from PendulumScene.tsx
...
// Match ARSAgent constructor exactly: init rng=mulberry32(42), ars rng=mulberry32(123)
...
console.log(`ARS final test: ${nP} params, exact ARSAgent seeds`);
...
for(let iter=0;iter<600;iter++){
  ...
  if(iter%20===0||iter===599){
    const uf=evalUpright();
    const el=((Date.now()-t0)/1000).toFixed(1);
    console.log(`${String(iter).padStart(5)} | ${rM.toFixed(3).padStart(8)} | ${(uf*100).toFixed(0).padStart(7)}% | ${el}s`);

// Comprehensive Åström-Furuta gain sweep for the pendubot.
// Tests robustness across a grid of (ke, kp, kd) on many random ICs
// drawn from a wide distribution. Reports per-tuning success rate
// and median time-to-basin. Goal: pick a single tuning that swings
// up reliably from anywhere the user might leave the pendulum.
...
// Run one episode, return either time-to-balance (s) or null on failure.
function runEpisode(ctrl, init, maxSeconds=15) {
  let s = { ...init };
  ...
  for (let i = 0; i < steps; i++) {
    ...
    if (d < 0.05 && Math.abs(s.w1) < 0.3 && Math.abs(s.w2) < 0.3) {
      return i * DT;
    }
  }
  return null;
...
// Initial-condition distribution: realistic states the user might
// leave the pendulum in after grabbing. Mostly hanging-ish, occasionally
// kicked, never starts in the basin.
function randomIC(rng) {
  const r = rng();
  if (r < 0.4) {
    // Near hanging rest with a small kick.
    return { th1: (rng()-0.5)*0.8, th2: (rng()-0.5)*0.8, w1: (rng()-0.5)*1, w2: (rng()-0.5)*1 };
  } else if (r < 0.7) {
    // Half-swung up.
    return { th1: (rng()-0.5)*1.5, th2: Math.PI/2 + (rng()-0.5)*1.5, w1: (rng()-0.5)*2, w2: (rng()-0.5)*2 };
  } else if (r < 0.9) {
    // Wide-angle, possibly already swinging.
    return { th1: (rng()-0.5)*Math.PI, th2: (rng()-0.5)*2*Math.PI, w1: (rng()-0.5)*4, w2: (rng()-0.5)*4 };
  } else {
    // Near inverted but outside the basin.
    return { th1: (rng()-0.5)*0.6, th2: Math.PI + (rng()-0.5)*1.6, w1: (rng()-0.5)*3, w2: (rng()-0.5)*3 };
  }
}

// Mulberry32 deterministic RNG so each tuning is graded on identical ICs.
...
function evalTuning(ke, kp, kd) {
  ...
  for (let i = 0; i < N_TRIALS; i++) {
    const ic = randomIC(rng);
    if (inBasin(ic)) { nSuccess++; times.push(0); continue; }
  ...
  return { rate: nSuccess/N_TRIALS, median, count: nSuccess };
}

console.log("=== Pendubot swing-up gain sweep ===");
console.log(`Trials per cell: ${N_TRIALS}\n`);
console.log("ke   kp   kd   | rate   median-t  (n)");
console.log("-".repeat(46));

const results = [];
for (const ke of [8, 12, 16, 20, 24, 30, 40]) {
  for (const kp of [1, 2, 3, 5]) {
    for (const kd of [0.2, 0.3, 0.5, 1.0]) {
      ...
      console.log(
        ...
        `${(r.rate*100).toFixed(0).padStart(3)}%   ${r.median===Infinity?"-":r.median.toFixed(1).padStart(5)+"s"}    (${r.count})`
      );
    }
  }
}

console.log("\n=== Top 8 tunings by (success rate, then median time) ===");
results.sort((a,b)=>{
...
for (const r of results.slice(0, 8)) {
  console.log(`ke=${String(r.ke).padStart(2)} kp=${String(r.kp).padStart(2)} kd=${r.kd.toFixed(1)}  ` +
    `rate=${(r.rate*100).toFixed(0)}%  median=${r.median.toFixed(1)}s`);
}

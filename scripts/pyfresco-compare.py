"""
Compare browser H∞ rst_init output against pyfresco's reference.

Plant: Dipole correction coil — G(s) = 1 / (R_s + s L), R_s=0.1, L=10e-3
Specs: Ts = 5e-4 s, desBw = 200 Hz, desMm = 0.5, desZ = 0.8
Controller order: n_r = n_s = n_t = 6, n_integrators = 1

This script imports OptAlgoIB directly, bypassing pyfresco's package
__init__ (which pulls in the CERN-internal `pyfgc` module).
"""
...
# Build a shim `pyfresco.obcd` package in sys.modules so `from pyfresco.obcd
# import X` resolves inside OptAlgoIB.py / solve.py without executing the real
# __init__ (which imports pyfgc).
...
# Our Plant 1 is so easy that every γ in [0.1, 10] is feasible; the bisection
# then raises "could not bracket". Push g_max up so the upper end is infeasible.
constants.g_max = 1e4
...
# Upstream hinf has an early-exit branch:
#     elif (g_lmi[it] < 1 and it > 2):
#         return {'gamma_opt': g_lmi[it]}
# which drops the controller coefficients whenever γ converges below 1
# after iteration 2.  Easy plants hit that path on the very next LMI
# step, so we lose R/S/T.  We patch the method to return the full
# controller (same shape as the normal exit).  Behaviour is otherwise
# identical — we just build R0,S0,T0 and GAIN from the last bis_sol
# before returning.
import cvxpy as _cp
import numpy as _np
...
def _hinf_patched(self, R0, S0, order, T_flag):
    ...
    while g_lmi[it - 1] - g_lmi[it] > constants.LMI_tol:
        ...
        # Always snapshot the latest feasible controller so the early-exit
        # branches can still return a full dict.
        ...
        if bis_sol['g-opt-bis'] > constants.g_thresh:
            # Infeasible — bail out with whatever the last feasible result was.
            break
        elif (g_lmi[it] < 1 and it > 2):
            # Original pyfresco exits with only gamma_opt here — we keep the
            # coefficients from the just-finished iteration.
            break
        elif (g_lmi[it] > g_lmi[it - 1] and it > 2):
            # γ started climbing — prior iteration was better, but the
            # upstream function still returns *this* iteration's result, so
...
class UiParams:
    """Minimal drop-in for pyfresco.obcd.props.UiParams."""
    def __init__(self):
        self.des_bw = 200.0
        self.des_z = 0.8
        self.des_mm = 0.5
...
# ---------- plant: Dipole correction coil ------------------------------
Rs, L, TAU_D = 0.1, 10e-3, 150e-6   # 150 us actuator delay
Ts = 5e-4
w_nyq = np.pi / Ts          # ~6283 rad/s
w_init = (Rs / L) / 100      # 10 rad/s / 100 = 0.1 rad/s


def plant_frf(w):
    return np.exp(-1j * w * TAU_D) / (Rs + 1j * w * L)

# --- grid A: pyfresco's default log grid (~282 pts for order-6) ---
nu = 3 * 6
...
# --- grid B: browser's linear grid (200 pts) --------------------
w_lin = np.linspace(w_init, w_nyq, 200)


def closed_loop_frf(w_eval, R, S_full, T_full, Ts):
    """Evaluate CL(jw) = G(jw)*T(e^-jwTs) / (G(jw)*R(e^-jwTs) + S_full(e^-jwTs))
    on an arbitrary frequency grid. MA = 1 here."""
    G = plant_frf(w_eval)

    def poly_z(coeffs, w):
        # Sum_i c_i * z^{-i}  with z = exp(j w Ts)
...
def run(w, label):
    print(f"\n========== {label} (Nw={len(w)}) ==========")
    G = plant_frf(w)
    MA = np.ones_like(w, dtype=complex)    # moving-average filter = 1
    up = UiParams()
...
def plot_closed_loops(log_sol, lin_sol, log_hinf_sol, browser_sol, out_path):
    # Dense log-spaced frequency grid for smooth plotting.
    w_plot = np.logspace(np.log10(w_init), np.log10(w_nyq), 1200)
    ...
    ax_mag.axvline(200, color="#16a34a", lw=0.8, ls=":", label="desired f_c = 200 Hz")
    ax_mag.set_ylabel("|T_cl|  [dB]")
    ax_mag.set_title(
        "Closed-loop Bode — dipole coil (pyfresco rst_init)\n"
        f"Ts={Ts*1e3:.2g} ms, des_bw=200 Hz, des_mm=0.5, des_z=0.8, order 6"
    )
    ax_mag.grid(True, which="both", alpha=0.3)
    ax_mag.legend(loc="lower left", fontsize=9)

    ax_phase.axvline(200, color="#16a34a", lw=0.8, ls=":")
...
if __name__ == "__main__":
    ...
    # Browser TS synthesis output after removing WD_CAP (captured via
    # `npx tsx scripts/hinf-dipole-usercase.ts`; Ts=5e-4, desBw=200,
    # desMm=0.5, desZ=0.8, order 6).  H∞ norm = 0.997, f_c = 208 Hz.
    browser_sol = {
        "R": np.array([28.003746, -34.145588,  6.499337, 11.634259, -24.617647, 12.718481]),
        "S_full": np.array([1.000000, -1.287263,  0.409872, -0.385364, -0.296083,  0.558838]),
        "T": np.array([7.267034, -0.093689, -2.354927, -2.747329, -1.610310, -0.368192]),
    }

    out = Path(__file__).parent / "pyfresco-dipole-closed-loop.png"
